app.controller("register", ["$scope", "$window", "$compile", "$timeout", "global", function($scope, $window, $compile, $timeout, global) {
    ...
    $scope.register = function(event) {
        ...
        if(event.which == 1 || event.which == 13) {
            ...
            if(validation) {
                ...
                let rest = {
                    "data": {
                        "_token": $("meta[name=\"csrf-token\"]").attr("content"), "contact": {
                            "email": $scope.contact.email.value
                        }, "name": $scope.name.value, "nucode": $scope.nucode.value, "password": {
