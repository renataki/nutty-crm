app.controller("license", ["$scope", "$window", "$compile", "$timeout", "global", function($scope, $window, $compile, $timeout, global) {
    ...
    $scope.$on("initializeTable", function() {
        ...
        let columns = [{
            ...
            "data": "user.total", "name": "user.total"
        }, {
            "data": "package.status", "name": "package.status"
        }, {
        ...
        let table = $("#license").DataTable({
            "ajax": {
                ...
                }, "datatype": "json", "type": "POST", "url": $scope.global.url.base + "/license/table/"
            },
            "columns": columns,
    ...
    $scope.delete = function(id, nucode, event) {
        ...
        let rest = {
            "data": {
                "_token": $("meta[name=\"csrf-token\"]").attr("content"), "nucode": nucode
    ...
    $scope.initializeInputData = function() {
        ...
        return {
            "_token": $("meta[name=\"csrf-token\"]").attr("content"), "nucode": $scope.nucode.value, "user": {
    ...
    $scope.update = function(event) {
        ...
        if(validation) {
            ...
            let rest = {
                "data": data, "url": $scope.global.url.base + "/license/update"
            };
            global.rest(rest, function(response) {
